...
final workOrderReferenceOptionsProvider = Provider<AsyncValue<List<ReferenceOption>>>((
  ref,
) {
...
final assetReferenceOptionsProvider = Provider<AsyncValue<List<ReferenceOption>>>((
  ref,
) {
...
final technicianReferenceOptionsProvider = Provider<AsyncValue<List<ReferenceOption>>>((
  ref,
) {
  ...
  return techniciansAsync.whenData((technicians) {
    final merged = [
      if (currentUser != null && technicians.every((u) => u.id != currentUser.id))
        currentUser,
      ...technicians,
    ];
    return merged
        .map(
          (user) =>
              ReferenceOption(id: user.id, label: user.fullName, subtitle: user.email),
        )
        .toList();
  });
});

final templateReferenceOptionsProvider = Provider<AsyncValue<List<ReferenceOption>>>((
  ref,
) {
  ...
  return inspectionsAsync.whenData((inspections) {
    final templates = inspections.where(
      (inspection) =>
          inspection.workOrderId == 'TEMPLATE' && inspection.templateName.isNotEmpty,
    );
    return templates
        .map(
